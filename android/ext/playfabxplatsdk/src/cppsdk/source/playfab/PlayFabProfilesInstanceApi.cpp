...
namespace PlayFabInternal
{
    ...
    void PlayFabProfilesInstanceAPI::SetAvatarUrl(
        SetAvatarUrlRequest& request,
        const ProcessApiCallback<SetAvatarUrlResponse> callback,
        const ErrorCallback errorCallback,
        void* customData
    )
    {
        ...
        auto reqContainer = std::unique_ptr<CallRequestContainer>(new CallRequestContainer(
            ...
            headers,
            jsonAsString,
            std::bind(&PlayFabProfilesInstanceAPI::OnSetAvatarUrlResult, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
            customData,
            this->settings));

        reqContainer->successCallback = std::shared_ptr<void>((callback == nullptr) ? nullptr : new ProcessApiCallback<SetAvatarUrlResponse>(callback));
        reqContainer->errorCallback = errorCallback;
    ...
    void PlayFabProfilesInstanceAPI::OnSetAvatarUrlResult(int httpCode, const std::string& result, const std::shared_ptr<CallRequestContainerBase>& reqContainer)
    {
        CallRequestContainer& container = static_cast<CallRequestContainer&>(*reqContainer);

        SetAvatarUrlResponse outResult;
        if (ValidateResult(outResult, container))
        {
            ...
            if (internalPtr != nullptr)
            {
                const auto callback = (*static_cast<ProcessApiCallback<SetAvatarUrlResponse> *>(internalPtr));
                callback(outResult, container.GetCustomData());
            }
        }
    }

    void PlayFabProfilesInstanceAPI::SetDisplayName(
        SetDisplayNameRequest& request,
        const ProcessApiCallback<SetDisplayNameResponse> callback,
        const ErrorCallback errorCallback,
        void* customData
    )
    {
        ...
        auto reqContainer = std::unique_ptr<CallRequestContainer>(new CallRequestContainer(
            ...
            headers,
            jsonAsString,
            std::bind(&PlayFabProfilesInstanceAPI::OnSetDisplayNameResult, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
            customData,
            this->settings));

        reqContainer->successCallback = std::shared_ptr<void>((callback == nullptr) ? nullptr : new ProcessApiCallback<SetDisplayNameResponse>(callback));
        reqContainer->errorCallback = errorCallback;
    ...
    void PlayFabProfilesInstanceAPI::OnSetDisplayNameResult(int httpCode, const std::string& result, const std::shared_ptr<CallRequestContainerBase>& reqContainer)
    {
        CallRequestContainer& container = static_cast<CallRequestContainer&>(*reqContainer);

        SetDisplayNameResponse outResult;
        if (ValidateResult(outResult, container))
        {
            ...
            if (internalPtr != nullptr)
            {
                const auto callback = (*static_cast<ProcessApiCallback<SetDisplayNameResponse> *>(internalPtr));
                callback(outResult, container.GetCustomData());
            }
        }
    }
